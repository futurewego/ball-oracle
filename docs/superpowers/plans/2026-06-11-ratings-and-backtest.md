...
Expected: 打印两届赛事的 Brier 与 LogLoss。判读：Brier 接近 0.20-0.22、LogLoss 接近 0.95-1.00 = 模型校准合格、可上线概率展示；明显更差 = 回炉（调时间衰减/训练窗/加洲际收缩）。
...
- **决策门**：Brier ≤ 0.22 → 模型可用，进数据层 + 概率展示；> 0.24 → 先调模型（时间衰减、训练窗、洲际收缩、§9.4 物理特征），别急着盖楼。
